# Spec: `squad info` Command

## References

- PRD §8 (Comandos del CLI) — `squad info` table entry
- PRD §7.4 (Modo interactivo manual) — info usage context
- PRD §13 (Registry inicial) — agent fields
...
- GIVEN a registry containing an agent with version, runtime, install method, and description
- WHEN the user runs `squad info <agent-id>`
- THEN the output contains the agent's ID, name, version, runtime, install method, and description

#### Scenario: Agent with runtime dependency

- GIVEN a registry containing an agent that requires "node" runtime
- WHEN the user runs `squad info <agent-id>`
- THEN the output includes the runtime dependency information
...
- GIVEN a registry that does not contain the requested ID
